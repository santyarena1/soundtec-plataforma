...
import { CHANGELOG_KIND_STYLE, displayChangelogVersion, formatChangelogDate, type ChangelogEntryView } from "@/lib/changelog";
        ...
        {entry.items.map((item, index) => {
          ...
          return (
            <li key={`${entry.id}-${index}`} className="flex items-start gap-2 text-sm leading-5">
              <span className={cn("mt-0.5 shrink-0 rounded px-1.5 py-0.5 text-[10px] font-bold tracking-wide", style.className)}>
                {style.label}
              </span>
              <span>{item.text}</span>
            </li>
